...
// Make sure globals exist so the IDE doesn't complain
if (!THREE) {
...
function addAnalysisInfo() {
    var elevationGraph = document.getElementById('elevation-graph');
    var slopeGraph = document.getElementById('slope-graph');
    var analyticsValues = document.getElementsByClassName('value');
    var analysis = THREE.Terrain.Analyze(terrainScene.children[0], heightmapOptions);
    ...
    var deviations = getSummary(analysis);
...
function Settings() {
    ...
    this.after = function (vertices, options) {
        if (scope.edgeDirection !== 'Normal') {
            (scope.edgeType === 'Box' ? THREE.Terrain.Edges : THREE.Terrain.RadialEdges)(
                    vertices,
                    options,
                    scope.edgeDirection === 'Up' ? true : false,
                    scope.edgeType === 'Box' ? scope.edgeDistance : Math.min(options.xSize, options.ySize) * 0.5 - scope.edgeDistance,
                    THREE.Terrain[scope.edgeCurve]
                    );
        }
    };
    ...
    window.rebuild = this.Regenerate = function () {
        ...
        var segments = parseInt(scope.segments, 10);

        var settingHeightmap = scope.heightmap === 'heightmap.png';

        heightmapOptions = {
            after: scope.after,
            easing: THREE.Terrain[scope.easing],
            heightmap: settingHeightmap ? heightmapImage : (scope.heightmap === 'influences' ? customInfluences : THREE.Terrain[scope.heightmap]),
            material: scope.texture === 'Wireframe' ? mat : (scope.texture === 'Blended' ? blend : gray),
            maxHeight: scope.maxHeight - 100,
            minHeight: -100,
            steps: scope.steps,
            stretch: true,
            turbulent: scope.turbulent,
            useBufferGeometry: false,
            xSize: scope.size,
            ySize: Math.round(scope.size * scope['width:length ratio']),
            xSegments: segments,
            ySegments: Math.round(segments * scope['width:length ratio']),
            _mesh: typeof terrainScene === 'undefined' ? null : terrainScene.children[0] // internal only
        };

        scene.remove(terrainScene);

        terrainScene = THREE.Terrain(heightmapOptions);

        applySmoothing(scope.smoothing, heightmapOptions);

        scene.add(terrainScene);

        skyDome.visible = sand.visible = water.visible = scope.texture !== 'Wireframe';

        var heightmapCanvas = document.getElementById('heightmap');

        if (heightmapCanvas) {
            heightmapOptions.heightmap = heightmapCanvas;
            THREE.Terrain.toHeightmap(terrainScene.children[0].geometry.vertices, heightmapOptions);
        }

        scope['Scatter meshes']();

        addAnalysisInfo(heightmapOptions);
    };

    function altitudeProbability(z) {
        if (z > -80 && z < -50)
            return THREE.Terrain.EaseInOut((z + 80) / (-50 + 80)) * scope.spread * 0.002;
        else if (z > -50 && z < 20)
            return scope.spread * 0.002;
        else if (z > 20 && z < 50)
            return THREE.Terrain.EaseInOut((z - 20) / (50 - 20)) * scope.spread * 0.002;
        return 0;
    }
    ...
    this['Scatter meshes'] = function () {
        ...
        // Calulate the spread option for use below
        if (scope.scattering === 'Linear') {
            spread = scope.spread * 0.0005;
            randomness = Math.random;
        } else if (scope.scattering === 'Altitude') {
            spread = scope.altitudeSpread;
        } else if (scope.scattering === 'PerlinAltitude') {
            spread = (function () {
                var h = THREE.Terrain.ScatterHelper(THREE.Terrain.Perlin, options, skip, threshold)();
                var hs = THREE.Terrain.InEaseOut(scope.spread * 0.01);
                return function (v, k) {
                    var rv = h[k],
                            place = false;
                    if (rv < hs) {
            ...
            spread = THREE.Terrain.InEaseOut(scope.spread * 0.01) * (scope.scattering === 'Worley' ? 1 : 0.5);
            randomness = THREE.Terrain.ScatterHelper(THREE.Terrain[scope.scattering], options, 2, 0.125);
        }

        var geo = terrainScene.children[0].geometry;

        // Decorations objects (trees in this case)
        terrainScene.remove(decoScene);

        decoScene = THREE.Terrain.ScatterMeshes(geo, {
            mesh: mesh,
            w: segments,
            h: Math.round(segments * scope['width:length ratio']),
            spread: spread,
            smoothSpread: scope.scattering === 'Linear' ? 0 : 0.2,
        ...
        if (decoScene) {
            if (scope.texture === 'Wireframe') {
                decoScene.children[0].material = decoMat;
            } else if (scope.texture === 'Grayscale') {
...
function setupDatGui() {
    ...
    heightmapFolder.add(settingObject, 'heightmap', ['Brownian', 'Cosine', 'CosineLayers', 'DiamondSquare', 'Fault', 'heightmap.png', 'Hill', 'HillIsland', 'influences', 'Particles', 'Perlin', 'PerlinDiamond', 'PerlinLayers', 'Simplex', 'SimplexLayers', 'Value', 'Weierstrass', 'Worley'])
            .onFinishChange(settingObject.Regenerate);

    heightmapFolder.add(settingObject, 'easing', ['Linear', 'EaseIn', 'EaseInWeak', 'EaseOut', 'EaseInOut', 'InEaseOut'])
            .onFinishChange(settingObject.Regenerate);

    heightmapFolder.add(settingObject, 'smoothing', ['Conservative (0.5)', 'Conservative (1)', 'Conservative (10)', 'Gaussian (0.5, 7)', 'Gaussian (1.0, 7)', 'Gaussian (1.5, 7)', 'Gaussian (1.0, 5)', 'Gaussian (1.0, 11)', 'GaussianBox', 'Mean (0)', 'Mean (1)', 'Mean (8)', 'Median', 'None'])
            .onChange(function (val) {
                applySmoothing(val, heightmapOptions);
                settingObject['Scatter meshes']();
                if (heightmapOptions.heightmap) {
                    THREE.Terrain.toHeightmap(terrainScene.children[0].geometry.vertices, heightmapOptions);
                }
            });

    heightmapFolder.add(settingObject, 'segments', 7, 127).step(1)
            .onFinishChange(settingObject.Regenerate);

    heightmapFolder.add(settingObject, 'steps', 1, 8).step(1)
            .onFinishChange(settingObject.Regenerate);

    heightmapFolder.add(settingObject, 'turbulent')
            .onFinishChange(settingObject.Regenerate);
    ...
    decoFolder.add(settingObject, 'texture', ['Blended', 'Grayscale', 'Wireframe'])
            .onFinishChange(settingObject.Regenerate);

    decoFolder.add(settingObject, 'scattering', ['Altitude', 'Linear', 'Cosine', 'CosineLayers', 'DiamondSquare', 'Particles', 'Perlin', 'PerlinAltitude', 'Simplex', 'Value', 'Weierstrass', 'Worley'])
            .onFinishChange(settingObject['Scatter meshes']);

    decoFolder.add(settingObject, 'spread', 0, 100).step(1).onFinishChange(settingObject['Scatter meshes']);

    decoFolder.addColor(settingObject, 'Light color').onChange(function (val) {
        skyLight.color.set(val);
    });

    var sizeFolder = gui.addFolder('Size');

    sizeFolder.add(settingObject, 'size', 1024, 3072).step(256)
            .onFinishChange(settingObject.Regenerate);

    sizeFolder.add(settingObject, 'maxHeight', 2, 300).step(2)
            .onFinishChange(settingObject.Regenerate);

    sizeFolder.add(settingObject, 'width:length ratio', 0.2, 2).step(0.05)
            .onFinishChange(settingObject.Regenerate);

    var edgesFolder = gui.addFolder('Edges');

    edgesFolder.add(settingObject, 'edgeType', ['Box', 'Radial'])
            .onFinishChange(settingObject.Regenerate);

    edgesFolder.add(settingObject, 'edgeDirection', ['Normal', 'Up', 'Down'])
            .onFinishChange(settingObject.Regenerate);

    edgesFolder.add(settingObject, 'edgeCurve', ['Linear', 'EaseIn', 'EaseOut', 'EaseInOut'])
            .onFinishChange(settingObject.Regenerate);

    edgesFolder.add(settingObject, 'edgeDistance', 0, 512).step(32)
            .onFinishChange(settingObject.Regenerate);

    gui.add(settingObject, 'Flight mode').onChange(function (val) {
        useFPS = val;
    ...
    gui.add(settingObject, 'Scatter meshes');

    gui.add(settingObject, 'Regenerate');
}
...
function applySmoothing(smoothing, o) {
    ...
    if (smoothing === 'Conservative (0.5)')
        THREE.Terrain.SmoothConservative(g, o, 0.5);
    if (smoothing === 'Conservative (1)')
        THREE.Terrain.SmoothConservative(g, o, 1);
    if (smoothing === 'Conservative (10)')
        THREE.Terrain.SmoothConservative(g, o, 10);
    else if (smoothing === 'Gaussian (0.5, 7)')
        THREE.Terrain.Gaussian(g, o, 0.5, 7);
    else if (smoothing === 'Gaussian (1.0, 7)')
        THREE.Terrain.Gaussian(g, o, 1, 7);
    else if (smoothing === 'Gaussian (1.5, 7)')
        THREE.Terrain.Gaussian(g, o, 1.5, 7);
    else if (smoothing === 'Gaussian (1.0, 5)')
        THREE.Terrain.Gaussian(g, o, 1, 5);
    else if (smoothing === 'Gaussian (1.0, 11)')
        THREE.Terrain.Gaussian(g, o, 1, 11);
    else if (smoothing === 'GaussianBox')
        THREE.Terrain.GaussianBoxBlur(g, o, 1, 3);
    else if (smoothing === 'Mean (0)')
        THREE.Terrain.Smooth(g, o, 0);
    else if (smoothing === 'Mean (1)')
        THREE.Terrain.Smooth(g, o, 1);
    else if (smoothing === 'Mean (8)')
        THREE.Terrain.Smooth(g, o, 8);
    else if (smoothing === 'Median')
        THREE.Terrain.SmoothMedian(g, o);
...
function customInfluences(g, options) {
    ...
    var height = options.maxHeight * 0.8;
...
function cleanAnalytic(val) {
    if (Array.isArray(val)) {
        if (val.length === 1) {
            ...
            var str = val.map(function (v) {
                return Math.round(v);
            }).join(', ');
            if (str.length > 9)
                str = val.join(',');
            if (str.length > 9)
                str = str.substring(0, str.lastIndexOf(',', 7)) + ',&hellip;';
            return str;
        }
    }
    var valIntStr = (val | 0) + '',
